...
import {AppGridColumnSizeSelector, AppGridRowSizeSelector, Grid, GridChildren, GridChildrenArea} from './styles';
...
export default function AppGrid() {
	...
	let {rowArr, colArr, setColArr, setRowArr, divNum, columnTemplate, rowTemplate, childArea, setChildArea} = useContext(GridContext);
	...
	function colSizeChangeHandler(index, event) {
		...
		setColArr(colArr.map((i, idx) => idx === index ? {unit: event.target.value} : i));
	}
	...
	function rowSizeChangeHandler(index, event) {
		...
		setRowArr(rowArr.map((i, idx) => idx === index ? {unit:event.target.value} : i));
	}
	...
	useEffect(() =>{
		if(Object.values(child).length && startEnd === 'e'){
			let [startrow, endrow] = child.srow <= child.erow ? [child.srow, child.erow] : [child.erow, child.srow];
			let [startcol, endcol] = child.scol <= child.ecol ? [child.scol, child.ecol] : [child.ecol, child.scol];
	...
	return (
		<main>
			<AppGridColumnSizeSelector
				...
				className={['colunits', columns > 8 ? 'widthfull' : '']}
			>
				{
